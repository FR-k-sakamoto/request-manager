...
10. Redirect handling (open redirects) + CORS + security headers (CSP, optional HSTS, etc.).
...
- SHOULD enable key `SecurityMiddleware` headers/settings: Referrer-Policy, COOP, nosniff, SSL redirect (with correct proxy configuration). HSTS should only be considered with explicit ops sign-off; see DJANGO-HTTPS-001. ([Django Project][3])
...
- SHOULD consider enabling:
  ...
  - MAY consider HSTS via `SECURE_HSTS_SECONDS` (+ includeSubDomains/preload as appropriate) only with explicit ops sign-off. ([Django Project][3])
...
- If HSTS is enabled incorrectly, it can break the site for the duration.
...
- If HSTS is explicitly requested and approved, add it carefully (start with low value, validate, then increase). Django warns misconfig can break your site for the HSTS duration. ([Django Project][3])
...
- SHOULD use `django.middleware.security.SecurityMiddleware` and configure it appropriately (production) for:
  ...
  - HTTPS redirects and (optional) HSTS only when explicitly requested (see DJANGO-HTTPS-001). ([Django Project][3])
